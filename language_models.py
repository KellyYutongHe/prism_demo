...
class DALLE(LanguageModel):
    API_RETRY_SLEEP = 10
    API_ERROR_OUTPUT = "$DALLEERROR$"
    API_QUERY_SLEEP = 0.5
    API_MAX_RETRY = 5
    API_TIMEOUT = 20
    # openai.api_key = os.getenv("OPENAI_API_KEY")
...
class StableDiffusion(LanguageModel):
    API_RETRY_SLEEP = 10
    API_ERROR_OUTPUT = "$DALLEERROR$"
    API_QUERY_SLEEP = 0.5
    API_MAX_RETRY = 5
    ...
    def generate(self, prompt: str):
        ...
        Returns:
            str: generated response
        '''
        output = self.pipe(prompt=prompt, num_inference_steps=1, guidance_scale=0.0).images[0]
        return output 
    
    def batched_generate(self, prompt_list: List[str]):
        return [self.generate(prompt) for prompt in prompt_list]
